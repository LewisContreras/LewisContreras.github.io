    ...
    return (
        <Box boxShadow="base" display="flex" alignItems="center" position="fixed" top="0" w="100vw"  bgColor="primary" zIndex="10" h={["80px","60px"]} >
          <HStack   w="90%" mx="auto"
            ...
            <Center display={["flex","none"]} className="scene_element scene_element--fadeinleft"
             onClick={()=>setMenu(!menu)} border="1px solid" borderColor="terciary" borderRadius="4px" p="5px" >
            ...
            <Link className="navbar-link"  _hover={{textDecoration:"none"}} _focus={{outline:"none"}}  href="#welcome" fontWeight="600" textAlign="center" color="secondary" borderBottom="3px solid" onClick={(e)=>{
              handleSmooth(e)
              handleSelected(e)
              }} >{language ? "Bienvenido" : "Welcome" }</Link>
            <Link className="navbar-link" _hover={{textDecoration:"none"}} _focus={{outline:"none"}} fontWeight="600" href="#projects"  color="terciary" onClick={(e)=>{
              handleSmooth(e)
              handleSelected(e)
              }}  >{language ? "Proyectos" : "Projects" }</Link>
            {/* <Link py="4px" width="100%" textAlign="center" borderTop="2px solid" borderColor="terciary" color="terciary" >Technologies</Link> */}
            <Link className="navbar-link" href="#contact" _hover={{textDecoration:"none"}} _focus={{outline:"none"}} fontWeight="600" color="terciary" onClick={(e)=>{
              handleSmooth(e)
              handleSelected(e)
              }}  >{language ? "Contacto" : "Contact" }</Link>
            </HStack>
            <HStack 
